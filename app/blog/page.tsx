...
import { ArrowRight, Trophy, Calendar, AlertOctagon, BarChart3, Database, Cable, Layers, CheckCircle } from 'lucide-react';
...
export default async function BlogPage(props: {
  ...
  const currentTag = typeof searchParams.tag === 'string' ? searchParams.tag : 'All';
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="mb-12 border-b border-black pb-8 flex items-start justify-between gap-4">
        <div>
          <h1 className="text-4xl md:text-5xl font-bold mb-4 tracking-tighter">The Archive.</h1>
          <p className="text-gray-600 font-light text-lg max-w-2xl">
            A collection of business insights, SAP best practices, and Power BI hacks.
          </p>
        </div>
      ...
      <Suspense fallback={<div className="h-10 w-full bg-gray-50 animate-pulse rounded-full"></div>}>
        <TagFilter initialTags={tags} />
      </Suspense>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {displayedPosts.map((post) => (
          <article key={post.slug} className="relative border border-gray-200 p-6 hover:border-black transition-all duration-300 bg-white hover:shadow-sm group h-full flex flex-col items-start">
              <div className="flex items-center gap-3 text-xs font-mono text-gray-600 mb-3 uppercase tracking-widest w-full relative z-20 flex-wrap">
                ...
                <span key={cat} className="flex items-center gap-1.5 text-black font-bold border border-black px-2 py-0.5 whitespace-nowrap">
                    {cat === 'Power BI' && <BarChart3 className="w-3 h-3" />}
                    {cat === 'SAP' && <Database className="w-3 h-3" />}
                    {cat === 'Integration' && <Cable className="w-3 h-3" />}
                    {cat === 'Extra Tasks' && <Layers className="w-3 h-3" />}
                    {cat === 'Wins' && <Trophy className="w-3 h-3" />}
                    {cat === 'Reflections' && <Calendar className="w-3 h-3" />}
                    {cat === 'Fails' && <AlertOctagon className="w-3 h-3" />}
                    {cat}
                </span>
                ))}
                </div>
