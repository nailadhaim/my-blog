...
export default function RootLayout({
  ...
  return (
    <html lang="en" className="scroll-smooth bg-white">
      ...
      >
        <div
          className="fixed inset-0 z-[0] bg-dot-black opacity-40 pointer-events-none"
          aria-hidden="true"
        />
        ...
        <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-black/5">
          <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
            <Link
              href="/"
              className="font-bold text-2xl tracking-tighter hover:opacity-70 transition-opacity"
            >
              Naila's Blog.
            </Link>
            <nav className="flex items-center gap-8 text-sm font-medium pr-2">
              <Link
                href="/"
                className="hover:text-black text-gray-700 transition-colors uppercase tracking-widest text-xs"
              >
                Home
              </Link>
              <Link
        ...
        <footer className="relative z-10 border-t border-gray-100 py-12 mt-20 bg-white">
          <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div className="text-xs text-gray-600 font-mono uppercase tracking-widest">
              © {new Date().getFullYear()} Naila's Blog. — Crafted with Next.js
              & Tailwind.
            </div>
          </div>
        </footer>
