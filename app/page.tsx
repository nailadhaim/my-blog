...
import { ArrowRight, Trophy, Calendar, AlertOctagon, BarChart3, Database, Cable, Layers } from 'lucide-react';
import HeroText from "@/components/hero-text";
import { getLatestPost } from "@/lib/posts-server"; // Correct import for server function

export default async function Home() {
  const latestPost = await getLatestPost();
  if (!latestPost) return <div className="text-center py-20 text-gray-500">No posts yet.</div>;
    ...
    <div className="space-y-32">
      ...
      <section className="relative">
        <div className="flex items-center justify-between mb-12">
          <h2 className="text-4xl font-bold tracking-tight">Latest Reflection.</h2>
          <Link href="/blog" className="group bg-black text-white px-6 py-3 rounded-full text-sm font-medium hover:bg-gray-800 transition-all hover:scale-105 inline-flex items-center shadow-lg hover:shadow-xl">
            View Archive <ArrowRight className="ml-2 w-4 h-4 group-hover:translate-x-1 transition-transform" />
          </Link>
        </div>

        <Link href={`/blog/${latestPost.slug}`} className="block group perspective-1000 relative">
              ...
              <div className="flex-1 space-y-6">
                 <div className="flex items-center gap-3 text-xs font-mono text-gray-600 uppercase tracking-widest">
                  <span className="flex items-center gap-1.5 text-black font-bold border border-black px-2 py-0.5 bg-white z-20">
                    {primaryCategory === 'Power BI' && <BarChart3 className="w-3 h-3" />}
                    {primaryCategory === 'SAP' && <Database className="w-3 h-3" />}
                    {primaryCategory === 'Integration' && <Cable className="w-3 h-3" />}
                    {primaryCategory === 'Extra Tasks' && <Layers className="w-3 h-3" />}
                    {primaryCategory === 'Wins' && <Trophy className="w-3 h-3" />}
                    {primaryCategory === 'Reflections' && <Calendar className="w-3 h-3" />}
                    {primaryCategory === 'Fails' && <AlertOctagon className="w-3 h-3" />}
                    {primaryCategory}
                  </span>
      ...
      <section className="space-y-8">
        ...
        <div className="bg-gray-50 p-8 rounded-sm border border-gray-100">
          <p className="text-lg leading-relaxed mb-8 max-w-3xl">
            Working in Business IT involves bridging the gap between technical solutions and operational needs. 
            At ADB Safegate, optimizing workflows with Power BI and SAP is key to data-driven decision making.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             <div className="bg-white p-6 border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                ...
                <p className="text-gray-600">Visualizing complex airport operations data to improve efficiency and turnaround times.</p>
             </div>
             <div className="bg-white p-6 border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                ...
                <p className="text-gray-600">Streamlining resource planning and integrating disparate data sources for a unified view.</p>
             </div>
          </div>
        </div>
      </section>
